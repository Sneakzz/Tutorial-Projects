...
namespace Engine
{
    public class Player : LivingCreature
    {
        ...
        public static Player CreatePlayerFromDatabase(int currentHitPoints, int maximumHitPoints,
            int gold, int experiencePoints, int currentLocationID)
        {
            Player player = new Player(currentHitPoints, maximumHitPoints, gold, experiencePoints);
        ...
        private void LootTheCurrentMonster()
        {
            ...
            // Give monster's loot item to the player
        ...
        public void UsePotion(HealingPotion potion)
        {
            ...
            // The player used their turn to drink the potion, so let the monster attack
            LetTheMonsterAttack();
        }
        ...
        public void AddItemToInventory(Item itemToAdd, int quantity = 1)
        {
            ...
            if (item == null)
            {
                // The player does not already have the item, so add it to the inventory, with a quantity of 1
                Inventory.Add(new InventoryItem(itemToAdd, quantity));
            }
            else
            {
                // The player has the item in their inventory, so increase the quantity by one
        ...
        public void RemoveItemFromInventory(Item itemToRemove, int quantity = 1)
        {
            ...
            if (item == null)
            {
                ...
                // Maybe raise an error for this situation.
            }
            else
            {
                // The player has the item in their inventory, so decrease its quantity
                item.Quantity -= quantity;
        ...
        private bool PlayerDoesNotHaveTheRequiredItemToEnter(Location newLocation)
        {
            return !HasRequiredItemToEnterThisLocation(newLocation);
        }
        ...
        private bool PlayerHasAllQuestCompletionItemsFor(Quest quest)
        {
            ...
            // If we get here, then the player must have all the required items and enough of them to complete the quest
            return true;
        }
