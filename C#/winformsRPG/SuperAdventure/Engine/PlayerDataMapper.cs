        ...
        public static void SaveToDatabase(Player player)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(_connectionString))
                {
                    ...
                    using (SqlCommand existingRowCountCommand = connection.CreateCommand())
                    {
                        ...
                        else
                        {
                            ...
                            using (SqlCommand updateSavedGame = connection.CreateCommand())
                            {
                                ...
                                // Pass the values from the player object to the SQL query using parameters.
                                // Using paramters helps make your program more secure.
                    ...
                    // The Quest and Inventory tables might have more, or less rows in the database than what the player has in their properties.
                    // So when we save the player's game, we will delete all the old rows and add in all new rows.
                    // This is easier than trying to add/delete/update each individual row.

                    // Deleting existing Quest rows
