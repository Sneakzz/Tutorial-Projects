...
namespace CSharpTutC
{
    class Program
    {
        ...
        OtherSchool:
            ...
            // ----- WHILE LOOP -----
            // You use the while loop when you want to execute as long as a condition is true
            ...
            while (i <= 10)
            {
                ...
                // If it returns 0 that means the value is even
                if (i % 2 == 0)
                {
                    i++;

                    // Continue skips the rest of the code and starts execution back
            ...
            // ToInt64,
            // ToString
            // And they can convert from any type to any other type

            // ----- EXCEPTION HANDLING -----
            // We use exception handling to catch errors that could crash our program
            double num1 = 5;
            double num2 = 0;

            // Code that could cause an error is surrounded by a try block
            try
            {
                Console.WriteLine("5 / 0 = {0}", DoDivision(num1, num2));
            }

            // We catch the error and warn the user rather then crash the program
        ...
        static double DoDivision(double x, double y)
        {
            if (y == 0)
            {
                // We are throwing an exception because you can't divide by zero
